...
class TelegramDBService:
    ...
    async def store_message(self, message: dict[str, Any]) -> bool:
        ...
        try:
            logger.debug("Starting to store message...")
            ...
            logger.debug(
                f"Message data after field removal: {list(message_data.keys())}"
            )
            ...
            if self.session:
                async with self.session.post(url, json=message_data) as response:
                    ...
                    logger.debug(
                        f"API Response for message {message_id}: Status {response.status}, Body: {response_text[:200]}"
                    )
                    ...
                    if response.status in [
                        ...
                        logger.warning(
                            f"Failed to store message {message_id}: {response.status} - {response_text}"
                        )
                        return False
            else:
    ...
    async def store_messages_bulk(
        self, messages: list[dict[str, Any]], skip_duplicates: bool = True
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for message in messages:
                try:
                    ...
                    if (
                        "message_id" in msg_copy
                        and "channel_username" in msg_copy
                        and "text" in msg_copy
                    ):
                        cleaned_messages.append(msg_copy)
                    else:
                        logger.warning(
                            f"Skipping message with missing required fields: {msg_copy}"
                        )
            ...
            params = {"skip_duplicates": skip_duplicates, "batch_size": 100}
            ...
            if cleaned_messages:
                ...
                for i, msg in enumerate(cleaned_messages):
                    ...
                    if boolean_fields:
                        logger.warning(
                            f"Message {i} contains boolean values: {boolean_fields}"
                        )
            ...
            if self.session:
                async with self.session.post(
                    url, json=cleaned_messages, params=params
                ) as response:
                    response_text = await response.text()
                    logger.debug(
                        f"Bulk import response: Status {response.status}, Body: {response_text[:500]}"
                    )

                    if response.status == 200:
                        result = await response.json()
                        logger.info(
                            f"Bulk import successful: {result.get('summary', {})}"
                        )
                        return result
                    else:
                        logger.warning(
                            f"Bulk import failed: {response.status} - {response_text}"
                        )
                    ...
                    if response.status == 200:
                        ...
                        return {
                            ...
                            "date_range": {
                                "start": "N/A",
                                "end": "N/A",
                            },  # Not available in basic stats
                            "media_messages": basic_stats.get("media_messages", 0),
                            "text_only_messages": basic_stats.get(
    ...
    async def get_messages_for_analysis(
        ...
        try:
            ...
            if limit is None:
                ...
                if total_messages > 0:
                    logger.info(
                        f"📊 Fetching ALL {total_messages:,} messages for comprehensive analysis..."
                    )
                    ...
                    if self.session:
                        async with self.session.get(url, params=params) as response:
                            if response.status == 200:
                                result = await response.json()
                                logger.info(
                                    f"✅ Successfully downloaded {len(result):,} messages"
                                )
                                return result
                            else:
                                logger.warning(
                                    f"Bulk export failed with status {response.status}, falling back to paginated approach"
                                )
            ...
            if limit:
                logger.info(
                    f"📊 Fetching {limit:,} messages using paginated approach..."
                )
                url = f"{self.db_url}/api/v1/telegram/messages"
                params = {
